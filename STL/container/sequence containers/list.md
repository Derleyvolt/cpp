...
As seguintes operações não invalidam iterators e referências para outros membros: `insert()`, `emplace()`, `emplace...()`, 
`push_front()`, `push_back`, `pop_front()` e `erase()`.                                                                           
...
c.insert(pos,n,elem)       |          Inserts n copies of elem before iterator position pos and  returns the position of the first element (or pos if there is no new element)
