...
• Sets e multisets pelo fato deles serem otimizados para busca rápida de elementos, eles fornecem funções de busca especiais.
  Essas funções são versões especiais de algoritmos gerais que possuem o mesmo nome. Você deve sempre preferir versões otimizadas
  para sets e multisets para alcançar complexidade logartica ao invés de complexidade linear de algortimos gerais
...
• Sets e multisets não permitem alterar o valor de um elemento, isso assegura que você não comprometer a ordem dos elementos mudando
  seus valores.
...
para alcançar complexidade logartica ao invés de complexidade linear de algortimos gerais. Por exemplo, a busca de uma coleção de 1000
...
c.equal_range(val)                 Returns a range with all elements with a value equal to val (i.e., the first and last position,
...
c.insert(val)                                 Inserts a copy of val and returns the position of the new element and, for sets, whether 
                                              it succeeded
c.insert(pos,val)                             Inserts a copy of val and returns the position of the new element (pos is used as a hint
                                              pointing to where the insert should start the search)
c.insert(beg,end)                             Inserts a copy of all elements of the range [beg,end) (returns nothing)
c.insert(initlist)                            Inserts a copy of all elements in the initializer list initlist (returns nothing; 
                                              since C++11)
c.emplace(args...)                            Inserts a copy of an element initialized with args and returns the position of the new
                                              element and, for sets, whether it succeeded (since C++11)
c.emplace_hint(pos,args...)                   Inserts a copy of an element initialized with args and returns the position of the new
                                              element (pos is used as a hint pointing to where the insert should start the search)
...
Para multisets, desde C++11 é garantido que insert(), emplace(), e erase() preservam a ordem relativa de elementos equivalentes, e que
inserção de elementos são colocados no final de valores equivalentes existentes. Note que os tipos de retorno das funções de inserção
insert() e emplace() diferem no seguinte:
